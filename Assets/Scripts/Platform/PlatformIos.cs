...
public class PlatformIos : IPlatformBase
{
    ...
    Action<CloudMetadata> onPeekResultSave;

    public static string LoginErrorTitle => "\\iCloud 로그인 필요".Localized();

    public static string LoginErrorMessage =>
        "\\홈 화면 -> 설정 -> (내 계정) -> iCloud에 로그인 해 주세요. 그 다음 Game Center, iCloud Drive, BalloonRider 항목을 켜고 다시 시도 해 주세요."
            .Localized();

    public static string ConfirmMessage => "\\확인".Localized();
    ...
    public bool CheckLoadSavePrecondition(string progressMessage, UnityAction onNotLoggedIn, UnityAction onAbort)
    {
        ...
        {
            ConfirmPopup.instance.OpenYesNoPopup(Platform.GetText("platform_game_center_login_required_popup"),
