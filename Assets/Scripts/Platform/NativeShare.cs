...
public static class NativeShare
{
    ...
    {
        ShareMultiple(body, new[] {filePath}, url, subject, mimeType, chooser);
    }
        ...
        bool chooser, string chooserText)
    {
        ...
        using (var intentObject = new AndroidJavaObject("android.content.Intent"))
        {
            ...
            if (!string.IsNullOrEmpty(url))
                ...
                using (intentObject.Call<AndroidJavaObject>("putExtra", intentClass.GetStatic<string>("EXTRA_STREAM"),
                    uriObject))
                {
                }
                ...
                using (var uris = new AndroidJavaObject("java.util.ArrayList"))
                {
                    for (var i = 0; i < filePaths.Length; i++)
                        ...
                        using (var uriObject =
                            uriClass.CallStatic<AndroidJavaObject>("parse", "file://" + filePaths[i]))
                        {
                            uris.Call<bool>("add", uriObject);
                        }

                    using (intentObject.Call<AndroidJavaObject>("putParcelableArrayListExtra",
                        intentClass.GetStatic<string>("EXTRA_STREAM"), uris))
                    {
                    }
                }
            ...
            using (var currentActivity = unity.GetStatic<AndroidJavaObject>("currentActivity"))
            {
                ...
                {
                    var jChooser =
                        intentClass.CallStatic<AndroidJavaObject>("createChooser", intentObject, chooserText);
