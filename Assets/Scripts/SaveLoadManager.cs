...
public class SaveLoadManager {
    ...
    public static string SaveFileName { get { return GetSaveLoadFilePathName(GetSaveSlot() + 1); } }
    public static string LoadFileName { get { return GetSaveLoadFilePathName(GetSaveSlot()); } }
    ...
    private static byte[] lastSaveDataArray;
    ...
    public static bool Save(BalloonSpawner spawner, ConfigPopup configPopup, Sound sound, Data data, SaveReason sr) {
        ...
        if (spawner.loadedAtLeastOnce == false) {
            Debug.LogWarning("****** Save() called before first Load(). There might be an error during Load(). Save() will be skipped to prevent losing your save data.");
            return false;
        }
    ...
    public static void Load(BalloonSpawner spawner) {
        ...
        List<System.Exception> exceptionList = new List<Exception>();
        ...
        for (int i = 0; i < maxSaveDataSlot; i++) {
            try {
                if (LoadInternal(spawner)) {
                    ...
                    // 어쩄든 실패긴 실패.
                    // 이전 슬롯으로 넘어간다.
                    exceptionList.Add(new System.Exception("Balloon Save Data Load Exception"));
        ...
        if (exceptionList.All(e => e.GetType() == typeof(SaveFileNotFoundException))) {
            ...
            var exception = (NotSupportedBalloonSaveDataVersionException)exceptionList.FirstOrDefault(e => e.GetType() == typeof(NotSupportedBalloonSaveDataVersionException));
    ...
    static bool LoadInternal(BalloonSpawner spawner) {
        ...
        if (balloonSaveData2.version < 1) {
            ...
            if (balloonSaveData2.version == LatestVersion) {
                ...
                throw new Exception($"[CRITICAL ERROR] Latest version {LatestVersion} not match save file latest version field {balloonSaveData2.version}!!!");
            }
            ...
            BalloonLogManager.Add(BalloonLogEntry.Type.GameLoaded, BalloonSpawner.instance.LastBalloonLevel, BalloonSpawner.instance.Gem);
            return true;
        }
    }
    ...
    public static void MigrateBalloonSaveData2(BalloonSaveData2 balloonSaveData2) {
        if (balloonSaveData2 == null) {
            throw new System.ArgumentNullException("balloonSaveData2");
        }
        ...
        // Version 1 --> 2
        if (balloonSaveData2.version == 1) {
            SushiDebug.LogFormat("Upgrading save file version from {0} to {1}", balloonSaveData2.version, balloonSaveData2.version + 1);
    ...
    private static string ProcessCriticalLoadErrorPrelude(List<System.Exception> exceptionList) {
        Debug.LogErrorFormat("Load: Unknown exception thrown: {0}", exceptionList[0].ToString());
        System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
        Debug.LogErrorFormat(t.ToString());
        return t.ToString();
    }

    public static void ProcessCriticalLoadError(List<System.Exception> exceptionList, string st) {
        BalloonLogManager.Add(BalloonLogEntry.Type.GameCriticalError, 0, 0);
        ChangeLanguageBySystemLanguage();
        ConfirmPopup.instance.OpenTwoButtonPopup("\\BalloonRider 저장 파일을 불러오는 중 오류가 발생했습니다.\\n더 심각한 데이터 유실을 막기 위해 게임 플레이는 중단됐습니다.\\n문제 해결을 위해서는 게임 진행 상황이 모두 담긴 파일을 업로드해야 합니다.\\n문제 분석 및 수정이 되면 게임을 재개할 수 있습니다.\\n업로드를 진행하시겠습니까?\\n\\n(인터넷 연결이 필요합니다.)".Localized(), () => UploadSaveFileAsync(exceptionList, st, false), () => AskAgainToReportSaveData(exceptionList, st), "\\중대한 오류 발생".Localized(), "\\예".Localized(), "\\아니요".Localized());
    }

    public static void ProcessUpdateNeededError(int saveFileVersion) {
        ChangeLanguageBySystemLanguage();
        ConfirmPopup.instance.Open(string.Format(@"\BalloonRider 업데이트가 필요합니다.\n\n확인을 눌러 업데이트하세요!\n\n저장 파일 버전: {0}\n클라이언트 지원 버전: {1}".Localized(), saveFileVersion, LatestVersion), () => {
            // 앱 상세 페이지로 보낸다.
            Platform.instance.RequestUserReview();
        });
    }

    private static void ChangeLanguageBySystemLanguage() {
    ...
    public static void EnterRecoveryCode(List<System.Exception> exceptionList, string st, bool notCriticalError) {
    ...
    private static void AskAgainToReportSaveData(List<System.Exception> exceptionList, string st) {
        ConfirmPopup.instance.OpenTwoButtonPopup(@"\진행 상황을 업로드하지 않으면 더이상 게임을 진행할 수 없습니다.\n\n업로드가 불가한 증상의 경우 네이버 공식 카페로 이동하여 해결 방법에 대해 문의해 주시기 바랍니다.".Localized(), () => { ConfigPopup.instance.OpenCommunity(); ProcessCriticalLoadError(exceptionList, st); }, () => SaveLoadManager.EnterRecoveryCode(exceptionList, st, false), "\\중대한 오류 발생".Localized(), "\\공식 카페 이동".Localized(), "\\복구 코드 입력".Localized());
    }

    private static async void UploadSaveFileAsync(List<System.Exception> exceptionList, string st, bool notCriticalError) {
        ConfirmPopup.instance.Close();
        await ErrorReporter.instance.UploadSaveFileIncidentAsync(exceptionList, st, notCriticalError);
    }

    private static void ProcessNewUser(BalloonSpawner spawner, System.Exception e) {
    ...
    private static void ShowFirstInstallWelcomePopup() {
    }

    private static int NewUserPseudoId() {
        return UnityEngine.Random.Range(100000, 1000000);
    }

    private static void ResetData(BalloonSpawner spawner) {
