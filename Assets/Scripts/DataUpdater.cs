...
public class DataUpdater : MonoBehaviour {
    static string BalloonBytesPath { get { return Path.Combine(Application.persistentDataPath, "Balloon.bytes"); } }
    static string BalloonBytesETagPath { get { return Path.Combine(Application.persistentDataPath, "Balloon.bytes.etag"); } }
    static string BalloonBytesHashPath { get { return Path.Combine(Application.persistentDataPath, "Balloon.bytes.hash"); } }
    ...
    public static Stream GetBuiltInBalloonDataStream() {
        TextAsset asset = Resources.Load("Data/Balloon") as TextAsset;
        var stream = new MemoryStream(asset.bytes);
        SushiDebug.Log("GetBalloonDataStream(): returning built-in datasheet");
        return stream;
    }

    public static IEnumerator GetCachedOrDownloadBalloonData(System.Action<float> onProgress, System.Action<bool> onSuccess, System.Action<string> onNetworkFail, System.Action onCacheCorrupt) {
        var getUri = string.Format("https://s3.ap-northeast-2.amazonaws.com/balloontycoon/{0}/Balloon.bytes", Application.version);
        SushiDebug.LogFormat("HTTP GET: {0}", getUri);
        using (UnityWebRequest www = UnityWebRequest.Get(getUri)) {
            www.redirectLimit = 1;
            www.SetRequestHeader("If-None-Match", GetCachedEtag());
            www.disposeDownloadHandlerOnDispose = true;
            www.downloadHandler = new DownloadHandlerBuffer(); // DownloadHandlerFile(Path.Combine(Application.persistentDataPath, "Balloon.bytes"));
    ...
    IEnumerator UpdateBalloonDataCoro() {
        yield return GetCachedOrDownloadBalloonData(progress => {
            ...
            ProgressMessage.instance.Close();
            DeleteAllCaches();
            ConfirmPopup.instance.OpenYesNoPopup(string.Format("게임 데이터에 문제가 있습니다. 다시 받으시겠습니까?"), () => {
