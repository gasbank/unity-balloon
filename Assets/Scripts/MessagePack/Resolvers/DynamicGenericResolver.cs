    ...
    internal static class DynamicGenericResolverGetFormatterHelper
    {
        ...
        internal static object GetFormatter(Type t)
        {
            ...
            if (t.IsArray)
            {
                ...
                if (rank == 2)
                    return Activator.CreateInstance(
                        typeof(TwoDimentionalArrayFormatter<>).MakeGenericType(t.GetElementType()));
                if (rank == 3)
                    return Activator.CreateInstance(
                        typeof(ThreeDimentionalArrayFormatter<>).MakeGenericType(t.GetElementType()));
                if (rank == 4)
                    return Activator.CreateInstance(
                        typeof(FourDimentionalArrayFormatter<>).MakeGenericType(t.GetElementType()));
            ...
            if (ti.IsGenericType)
            {
                ...
                if (isNullable && nullableElementType.GetTypeInfo().IsConstructedGenericType() &&
