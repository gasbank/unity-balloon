...
namespace MessagePack
{
    ...
    public static partial class LZ4MessagePackSerializer
    {
        ...
        public static int SerializeToBlock<T>(ref byte[] bytes, int offset, T obj, IFormatterResolver resolver)
        {
            ...
            MessagePackBinary.EnsureCapacity(ref bytes, offset,
                6 + 5 + maxOutCount); // (ext header size + fixed length size)
            ...
            // write body
            var lz4Length = LZ4Codec.Encode(serializedData.Array, serializedData.Offset, serializedData.Count, bytes,
                offset, bytes.Length - offset);

            // write extension header(always 6 bytes)
            extHeaderOffset +=
                MessagePackBinary.WriteExtensionFormatHeaderForceExt32Block(ref bytes, extHeaderOffset,
                    ExtensionTypeCode, lz4Length + 5);
        ...
        {
            if (serializedData.Count < NotCompressionSize) return serializedData;
            ...
            // write body
            var lz4Length = LZ4Codec.Encode(serializedData.Array, serializedData.Offset, serializedData.Count, buffer,
                offset, buffer.Length - offset);

            // write extension header(always 6 bytes)
            extHeaderOffset +=
                MessagePackBinary.WriteExtensionFormatHeaderForceExt32Block(ref buffer, extHeaderOffset,
                    ExtensionTypeCode, lz4Length + 5);
        ...
        /// <summary>
        ///     Get the war memory pool byte[]. The result can not share across thread and can not hold and can not call
        ///     LZ4Deserialize before use it.
        /// </summary>
...
namespace MessagePack.Internal
{
    internal static class LZ4MemoryPool
    {
        ...
        {
            if (lz4buffer == null) lz4buffer = new byte[LZ4Codec.MaximumOutputLength(65536)];
            return lz4buffer;
        }
    }
}