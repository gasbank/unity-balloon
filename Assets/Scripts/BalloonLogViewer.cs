...
public class BalloonLogViewer : MonoBehaviour {
    ...
    [SerializeField] Text pageText = null;
    [SerializeField] int curPage = 0;
    [SerializeField] int pages = 0;
    [SerializeField] int entryCountPerPage = 20;
    [SerializeField] int totalEntryCount = 0;
    [SerializeField] GameObject loadRemoteLogButton = null;
    ...
    private void FlushAndUpdateTotalPages() {
        logSource.Flush();
            ...
            try {
                using (var httpClient = new HttpClient()) {
                    ...
                    if (getTask.IsSuccessStatusCode) {
                        ...
                        var userPlayLogDataFieldsSaveDataStringValue = userPlayLogDataFieldsSaveData["bytesValue"] as string;
                        var userPlayLogUncompressedSizeData = userPlayLogDataFields["playLogUncompressedSizeData"] as Dict;
                        var userPlayLogUncompressedSizeDataIntegerValue = int.Parse(userPlayLogUncompressedSizeData["integerValue"] as string);
                        ...
                        MessagePack.LZ4.LZ4Codec.Decode(playLogData, 0, playLogData.Length, playLogUncompressedData, 0, playLogUncompressedData.Length);

                        readLogStream = new MemoryStream(playLogUncompressedData);

                        SushiDebug.LogFormat("Play Log Data Base64 ({0} bytes): {1}", playLogDataBase64 != null ? playLogDataBase64.Length : 0, playLogDataBase64);
    ...
    public void LoadRemoteLog() {
        ConfirmPopup.instance.OpenInputFieldPopup("Play Log Error Device ID", async () => {
            ...
            ConfirmPopup.instance.Close();
        }, "Remote Log", ConfirmPopup.Header.Normal, "", "");
    }
}
